...
class CaptureOutput(list):
    ...
    The captured output is available via the `_stderr` attribute (StringIO) for backward
    compatibility with existing code that calls `output._stderr.getvalue()`. The object
    itself is also a list containing captured lines.
    """
    ...
    def __enter__(self):
        ...
        # Create a capture buffer and redirect both stdout and stderr to it
    ...
    def __exit__(self, exc_type, exc, tb):
        # Extend the list with captured lines for easy iteration
...
def parse_run_statistics(log_lines):
    """
    Parse a 'Statistics for model' block from the log lines and return a DataFrame
    with columns 'Key' and 'Value'. If no statistics block is found, return an empty DataFrame.
    """
    stats = []
    is_extracting = False
    for line in log_lines:
        if "Statistics for model" in line:
            is_extracting = True
            continue
        if is_extracting:
            stripped = line.strip()
            # Stop extraction on blank line (end of statistics section)
            if not stripped:
                break
            # Only process lines with a key:value pattern
            if ":" in stripped:
                key, value = stripped.split(":", 1)
                stats.append({"Key": key.strip(), "Value": value.strip()})
            else:
...
def save_to_excel(results: dict, filename: str) -> None:
    with pd.ExcelWriter(filename, engine="openpyxl") as writer:
        ...
        for idx, (model_name, data) in enumerate(results.items(), start=1):
            # 準備變數和值數據
            df = pd.DataFrame({"Variable": data["Variable"], "Value": data["Value"]})

            # 捕獲的日誌輸出，分段保存
            df_log = pd.DataFrame({"Log": data["Log"]})

            # Parse run statistics block from the captured log (if present).
            df_run_statistics = parse_run_statistics(data["Log"])
            # Ensure Key/Value columns exist even when there are no statistics, so Excel has headers.
            if df_run_statistics.empty:
                df_run_statistics = pd.DataFrame(columns=["Key", "Value"])
            ...
            # 寫入數據到不同的工作表
            df.to_excel(
                writer, sheet_name=f"Model_{idx}_Solution", index=False, startrow=1
            )
            df_log.to_excel(
                writer, sheet_name=f"Model_{idx}_Log", index=False, startrow=1
            )
            df_run_statistics.to_excel(
                writer,
                sheet_name=f"Model_{idx}_Run_Statistics",
                index=False,
                startrow=1,
            )
            ...
            sheet = writer.sheets[f"Model_{idx}_Run_Statistics"]
            sheet.cell(row=1, column=1, value=f"Model File: {model_name}")
...
def main():
    ...
    def add_file_entry():
        ...
        button_browse = tk.Button(
            frame, text="瀏覽...", command=lambda: select_files(model_files)
        )
    ...
    def select_files(model_files):
        file_paths = filedialog.askopenfilenames(
            filetypes=[("Model Files", "*.lp *.mps *.json")]
        )
        if file_paths:
        ...
        try:
            ...
            for i, file_path in enumerate(model_file_paths):
                ...
                temp_file = await loop.run_in_executor(
                    None, execute_model_from_file, file_path, time_limit, temp_dir
                )
                ...
                progress_label.config(
                    text=f"{i + 1}/{len(model_file_paths)} ({(i + 1) / len(model_file_paths) * 100:.2f}%)"
                )
    ...
    time_limit_label = tk.Label(
        time_limit_frame, text="單一模型最大執行時間 (秒，0表示不限制):"
    )
    ...
    button_add_file = tk.Button(
        button_frame, text="添加模型路徑輸入框", command=add_file_entry
    )
    button_add_file.grid(row=0, column=0, padx=5, pady=5)

    button_solve = tk.Button(
        button_frame, text="求解模型", command=lambda: start_solve_thread()
    )
    button_solve.grid(row=0, column=1, padx=5, pady=5)

    save_button = tk.Button(
        button_frame,
        text="將結果輸出為 Excel 檔（.xlsx）",
        command=save_result_to_excel,
        state=tk.DISABLED,
    )
